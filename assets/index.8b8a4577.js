    ...
    if (emailValue && passwordValue && nameValue && lastnameValue && passwordConfirmValue) {
      ...
      if (errorCode) {
        if (errorCode === "auth/email-already-in-use") {
          ...
          message = "La contrase\xF1a debe tener minimo 6 caracteres";
...
async function onNavigate(pathname) {
...
async function validationloginWithMail(email, password) {
  if (email && password) {
    try {
      await emailLogin(email, password);
    } catch (error) {
      let message = "Algo sali\xF3 mal";
...
async function validationLoginWithGoogle() {
  try {
