...
function createUserDoc(user, fullName, photo) {
  return Ul(Aa(dataBase, "usuarios", user.uid), {
    id: user.uid,
    correo: user.email,
    nombre: fullName,
    foto: photo
...
function createPost(userId, displayName, postContent) {
  const postId = createID("post");
  console.log(postId);
  const today = new Date();
  return Ql(Ta(dataBase, "publicaciones"), {
    postId,
    userId,
    displayName,
...
function deletePost(idPost) {
  Gl(Aa(dataBase, "publicaciones", idPost));
}
function getUserPosts(userId) {
  const ref = Ta(dataBase, "publicaciones");
  const q2 = sl(
    ref,
    hl("date", "desc")
  );
...
function updatePost(postId, newFields) {
  return Kl(Aa(dataBase, "publicaciones", postId), newFields);
}
const getUserFromFirestore = (userId) => {
  const ref = Aa(dataBase, "usuarios", userId);
  return Ol(ref);
};
...
function counterLike(userUid, docPost, idButton) {
  const postLikes = Aa(dataBase, "publicaciones", docPost.uid);
  const ref = Ta(dataBase, "publicaciones");
  const arrLike = sl(ref, rl("postId", "==", docPost.postId));
  const getData = Bl(arrLike);
  let dataLikes = "";
  let cantLikes;
  getData.then((result) => {
  ...
  const q2 = sl(Ta(dataBase, "publicaciones"), rl("postId", "==", docPost.postId));
  jl(q2, (querySnapshot) => {
...
function updateteComments(userUid, docPost, idButton) {
  const q2 = sl(Ta(dataBase, "publicaciones"), rl("postId", "==", docPost.postId));
  jl(q2, (querySnapshot) => {
    querySnapshot.forEach((d) => {
...
function paintComments(arrayComments, idButton) {
...
function comments(post, containerRender) {
  ...
  getUserFromFirestore(user.uid).then((result) => {
    ...
    createPostForm.addEventListener("submit", (e) => {
      ...
      if (postInput.value.trim() !== "") {
        createPost(user.uid, user.displayName, postInput.value).then((result2) => {
